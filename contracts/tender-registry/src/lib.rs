...
#[contract]
pub struct TenderRegistryContract;

#[contractimpl]
impl TenderRegistryContract {
...
mod test {
    ...
    fn test_initialize() {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, TenderRegistryContract);
        let client = TenderRegistryContractClient::new(&env, &contract_id);
